...
class White(Gen_noise):
    def __init__(self, ener, device, is_mse, power_distribution_name):
        ...
        if power_distribution_name == 'invariant':
            self.power_generator = lambda x, b: x * torch.ones(1, 1, 1, 1,device=device)
    ...
    def generate_noise(self, H):
        ...
        b = H.shape[0]
        if self.is_mse:
            return noise * torch.sqrt(self.power_generator(self.ener, b))
        else:
    ...
    def generate_noise(self, H):
        ...
        if not self.variant_path:
            ...
            if self.variant_power:
                ...
                noise = noise * math.sqrt((self.ener * 2) / power)
        if self.is_mse:
            return noise
        else:
...
class Doppler(Gen_noise):
    def __init__(self, device, Doppler_window_length, alpha, alpha_fixed, num_car):
        ...
        self.kernel_power_mask = torch.cat((self.main_lobe_pos - torch.arange(self.main_lobe_pos + 1),
                                            torch.arange(self.main_lobe_pos)+1), dim=0).to(device)
...
class Salt(Gen_noise):
    def __init__(self, ener, device, power_distribution_name):
        super().__init__(ener, device, False)
        if power_distribution_name == 'invariant':
            self.power_generator = lambda x, b: x
        elif power_distribution_name == 'uniform':
            self.power_generator = lambda x, b: torch.rand(b, 1, 1, 1, device=device) * x
        else:
            raise NotImplementedError
        self.additive = False
    ...
    def generate_noise(self, H):
        b = H.shape[0]
        mapping = self.power_generator(self.ener, b) * torch.ones_like(H[:, :, :, 0])
        return 1 - torch.bernoulli(mapping).unsqueeze(3)
...
class Noise_Adder:
    ...
    def add_noise(self, H):
        if self.additive:
            ...
            gen = self.gens[self.counter]
            if gen.additive:
                H_out = H + gen.generate_noise(H)
            else:
                H_out = H * gen.generate_noise(H)
            self.counter += 1
...
def get_noise_adder(noise):
    ...
    if 'salt' in name:
        param = noise['salt']
        noise_adder.add_gen(Salt(param['ener'], torch.device(noise['device']),
                                 param['power_distribution_name']))
    return noise_adder
